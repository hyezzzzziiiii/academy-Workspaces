...
<script type = "text/javascript"> //자바스크립드 이용해서 확인함
function joinCheck(){
	if(document.frm.name.value.length==0){
		alert("이름을 써주세요"); frm.name.focus(); return false;
	}	
	if(document.frm.pwd.value ==""){
		alert("암호는 반드시 입력하셔야합니다."); frm.pwd.focus(); return false;
	}	
	if(document.frm.pwd.value!= document.from.pwd_check.value){
...
<!-- update.do는 매핑된 url -->
	<table>
		...
		<tr><td>아이디</td><td>${loginUser.userid} <!-- 여기 한번 확인 -->
		...
		<tr><td>전화번호</td><td>
		<input type = "text" name = "phone" value = "${loginUser.phone}"></td></tr>
