...
public class MemberServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 1. 현재 프로젝트에 Action 이라는 인터페이스를 제작합니다(추상메서드 execute) - 제작완료
		
		// 2. 더불어 각 command 에 따른 명령을 담은 java class 를 제작합니다
		// 3. 각 클래스는 모두 Action 인터페이스를 상속(구현)하여 제작합니다
		//     이는 Action 인터페이스에 execute 라는 추상 메서드가 있고, 제작되는 각 클래스는
		//     그 기능을 오버라이드하는 execute 메서드에 모든 실행명령을 기술합니다. - 제작완료
		
		// 4. 현재의 doGet 에 전달된 command 의 value 에 따라 해당 기능을 갖고 있는
		//     클래스를 Action 인터페이스의 레퍼런스 변수에 할당합니다(부모레퍼런스<-자식인스턴스)
		
		// 클래스 객체를 직접 생성  클래스레퍼런스<-클래스인스턴스, 클래스레퍼런스로 execute 실행
		//Login_Form_Action lfa = new Login_Form_Action();
		//lfa.execute(request, response);
		
		// 인터페이스 레퍼런스에 클래스 인스턴스를 넣는 경우(클래스 레퍼런스 생성후 할당)
		// 인터페이스 레퍼런스로 execute 실행
		//Action ac = null;
		//Login_Form_Action lfa = new Login_Form_Action();
		//ac = lfa;
		// 클래스 레퍼런스 생성없이 직접 인터페이스에 뉴인스턴스 할당
		// ac = new Login_Form_Action();
		// ac.execute(request, response);
		
		// command 에 따라 클래스를 선별하여 할당
		//if( command.equals("login_form")) ac = new Login_Form_Action();
		//else if(command.equals("join_form")) ac = new Join_Form_Action();
		// ac.execute(request, response);
		
		// 클래스의 인스턴스를 인터페이스 레퍼런스에 할당하는 작업을 별도 클래스의 메서드로 대체
		ActionFactory af = ActionFactory.getInstance();
		Action ac = af.getAction(command);
		
		// 5. 그리고 Action 에서 오버라이드된 execute 메서드를 실행합니다.
		ac.execute(request, response);
