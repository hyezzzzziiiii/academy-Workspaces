...
public class BoardListAction implements Action{
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 파라미터 "page"로 전달된 값이 있다면 그값으로 page 값을 변경합니다
		if( request.getParameter("page") != null )  
			page = Integer.parseInt(request.getParameter("page"));
		
		Paging paging = new Paging();  // 페이징 객체 생성
		paging.setPage(page);    // 현재 보고자 하는 페이지값을   멤버 변수에 전달 저장
		
		BoardDao bdao = BoardDao.getInstance();
		
		int count = bdao.getAllCount();  // 게시물의 갯수를 리턴해주는 메서드 호출
		System.out.println("Count : " + count);
		paging.setTotalCount(count);    // 리턴받은 총게시물 갯수를 setTotaCount 메서드에 전달 저장
